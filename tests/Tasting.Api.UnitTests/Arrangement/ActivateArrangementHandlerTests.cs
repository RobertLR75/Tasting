...
public sealed class ActivateArrangementHandlerTests
{
    ...
    public async Task HandleAsync_TransitionsToActive_WhenCreated()
    {
        ...
        var result = await handler.HandleAsync(new ActivateArrangementCommand(arrangement.Id, 0u));
    ...
    public async Task HandleAsync_ThrowsConflict_WhenWrongStatus()
    {
        ...
        await Assert.ThrowsAsync<ConflictException>(() =>
            handler.HandleAsync(new ActivateArrangementCommand(arrangement.Id, 0u)));
    }
    ...
    public async Task HandleAsync_ThrowsConflict_WhenRowVersionMismatch()
    {
        ...
        await Assert.ThrowsAsync<ConflictException>(() =>
            handler.HandleAsync(new ActivateArrangementCommand(arrangement.Id, 99u)));
    }
    ...
    public async Task HandleAsync_ThrowsNotFound_WhenMissing()
    {
        ...
        await Assert.ThrowsAsync<ServiceNotFoundException>(() =>
            handler.HandleAsync(new ActivateArrangementCommand(Guid.NewGuid(), 0u)));
    }
